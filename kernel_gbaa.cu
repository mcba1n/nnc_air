...
int main()
{
    std::cout << "air_sim_gbaa" << std::endl;
    std::default_random_engine generator;
    //std::srand(std::time(NULL));

    std::ofstream datafile;
    datafile.open("nanopore_model2/data_test.csv");
    ...
    for (int sim_idx = 0; sim_idx < N_dps; sim_idx++) {
        ...
        std::vector<std::vector<double>> P = read_data("nanopore_model2/P.csv", 0);//"nanopore_model2/P_gbaa/P_" + std::to_string(sim_idx + 1) + ".csv", 0);//"nanopore_model/P.csv", 0);
