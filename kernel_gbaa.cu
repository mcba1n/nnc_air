...
int main()
{
    ...
    datafile.open("nanopore_model2/data_gbaa.csv");

    // Channel mapping
    std::vector<std::vector<double>> f = read_data("nanopore_model2/f.csv", 0);
    thrust::host_vector<double> fh(Ns);
    for (int i = 0; i < Ns; i++) fh[i] = f[i][0];

    // Duration probabilities
    std::vector<std::vector<double>> P_K = read_data("nanopore_model2/P_K.csv", 0);
    int k_max = P_K.size();
    ...
    // Run simulations
    const double sigma_min = 0.05;//std::numeric_limits<double>::epsilon();
    ...
    for (int sim_idx = 0; sim_idx < N_dps; sim_idx++) {
        ...
        std::vector<std::vector<double>> P = read_data("nanopore_model2/P_gbaa/P_" + std::to_string(sim_idx + 1) + ".csv", 0);//"nanopore_model2/P.csv", 0);
        int Ns = P[0].size();
        ...
        //intervals = periodic_markers(m, W, T, E_K, t_seq, 2);
...
__global__ void gfa_init(int Ns, int Nd, int max_int_len, double* y, double* F,
    ...
    // GPU should not run this kernel if t_shifted is outside of its interval
    // otherwise it gives "illegal memory access" error
...
__global__ void gfa_slice(int ell, int Ns, int Nd, int max_int_len, double* y, double* Fp, double* Fc, 
    ...
    // GPU should not run this kernel if t is outside of its interval
    // otherwise it gives "illegal memory access" error
